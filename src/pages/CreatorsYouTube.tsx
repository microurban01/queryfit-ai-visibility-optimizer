...
const CreatorsYouTube: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const results = await YouTubeService.searchChannels(searchQuery, apiKey);
      setChannels(results);
    } catch (error) {
      console.error(error);
      actions.showToast({ title: 'Search Failed', message: 'Could not fetch YouTube channels. Check your API quota or key.', type: 'error' });
    } finally {
  ...
  const handleGenerateReport = async (channel: YouTubeChannel) => {
    ...
    setReportData(null);
    try {
      const report = await YouTubeService.getInfluencerReport(channel.id, channel.title, channel.customUrl);
